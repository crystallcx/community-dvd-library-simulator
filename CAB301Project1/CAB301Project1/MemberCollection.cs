...
class MemberCollection : IMemberCollection
{
    ...
    public bool Search(IMember member)
    {
        ...
        if (!IsEmpty()){
            while (i <= j){
                ...
                if (temp.CompareTo(members[mid]) == 0){
                    Console.WriteLine($"Member Exists: {members[mid].FirstName} {members[mid].LastName} --- SEARCH SUCCESS");
                    return true;
                }
        ...
        Console.WriteLine($"Member {member.FirstName} {member.LastName} does not exists -- SEARCH ERROR ");
        return false;
    }
    ...
    //THIS IS USED TO PRINT OUT A MEMBER ARRAY TO CONSOLE
